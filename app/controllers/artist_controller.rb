  ...
  def songs
    ...
    if artists.length == 1
      ...
      if sim == true && artists[0].present?
        ...
        lookup_data = JSON.parse(open("#{Artist::TINYSONG_BASE_URL}artist:#{artists[0].to_url}?format=json&key=#{Artist::TINYSONG_API_KEY}").read) if base_artist.nil?
        unless lookup_data == []
    ...
    artists.each do |instance|
      ...
      lookup_data = JSON.parse(open("#{Artist::TINYSONG_BASE_URL}artist:#{instance.to_url}?format=json&key=#{Artist::TINYSONG_API_KEY}").read) if @artist.nil?
